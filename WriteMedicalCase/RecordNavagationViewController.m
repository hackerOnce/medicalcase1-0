...
-(void)loadDoctorInfoWithDoctorID:(NSString*)doctorID
{
    ...
    NSString *did = doctorID;
    [MessageObject messageObjectWithUsrStr:did pwdStr:@"test" iHMsgSocket:self.socket optInt:2017 dictionary:NSDictionaryOfVariableBindings(did) block:^(IHSockRequest *request) {
        ...
        if ([request.responseData isKindOfClass:[NSDictionary class]]) {
            ...
            if ([tempDict.allKeys containsObject:@"dID"]) {
                dID = [tempDict objectForKey:@"dID"];
            }
            if ([tempDict.allKeys containsObject:@"dName"]) {
                dName = [tempDict objectForKey:@"dName"];
            }
            if ([tempDict.allKeys containsObject:@"dProfessionalTitle"]) {
                dProfessionalTitle = [tempDict objectForKey:@"dProfessionalTitle"];
            }
            if ([tempDict.allKeys containsObject:@"dept"]) {
                dept = [tempDict objectForKey:@"dept"];
            }
            if ([tempDict.allKeys containsObject:@"medicalCaseTeam"]) {
                medicalCaseTeam = [tempDict objectForKey:@"medicalCaseTeam"];
            }
            NSDictionary *doctorDict = @{@"dID":dID,@"dName":StringValue(dName),@"dProfessionalTitle":StringValue(dProfessionalTitle),@"dept":StringValue(dept),@"medicalCaseTeam":StringValue( medicalCaseTeam)};
            
            TempDoctor *doctor = [TempDoctor setSharedDoctorWithDict:doctorDict];
            NSLog(@"doctor name: %@",doctor.dName);
            
            if ([doctor.dProfessionalTitle isEqualToString:@"医师"]||[doctor.dProfessionalTitle isEqualToString:@""]) {
                [self loadModel];
               ...
               // NSMutableArray *array = [[NSMutableArray alloc] init];
                [self.classficationArray insertObject:@"待审核" atIndex:0];
...
-(void)loadModel
{
    ...
    for(int i = 0;i< self.classficationArray.count ;i++)
    {
        ...
        headview.section = i;
        NSString *titleString = [self.classficationArray objectAtIndex:i];
        [headview.backBtn setTitle:titleString forState:UIControlStateNormal];
        ...
        if ([titleString isEqualToString:@"待审核"]) {
            headview.open = NO;
        }
...
{
    [[NSUserDefaults standardUserDefaults] setObject:dID forKey:@"dID"];
    [[NSUserDefaults standardUserDefaults] setObject:@"张三" forKey:@"dName"];
    
    //@"2225"
    [MessageObject messageObjectWithUsrStr:@"2225" pwdStr:@"test" iHMsgSocket:self.socket optInt:2015 dictionary:@{@"did":dID} block:^(IHSockRequest *request) {
